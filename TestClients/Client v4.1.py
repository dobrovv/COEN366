...
class Client:
    ...
    def set_client_IP_address(self, ipaddress):
        self.ipaddress = ipaddress
    ...
    def set_client_TCP_port(self, tcp):
        self.tcp = int(tcp)
    ...
    def execServerCommand(self, cmd: list):
        ...
        if command == "REGISTER":
            ...
            if resp is None or len(resp) < 2:
                print('REGISTER : Invalid response', resp)
                return
            ...
            resp = sendServerCommand(cmd, sock)
            if resp is None or len(resp) < 2:
                print('AUTHENTICATE : Invalid response', resp)
                return
            ...
            resp = sendServerCommand(cmd, sock)
            if resp is None or len(resp) < 2:
                print('UPDATE-CONTACT : Invalid response', resp)
                return
            ...
            if command == "DE-REGISTER":
                ...
                try:
                    ...
                    resp = sendServerCommand(cmd, sock)
                    if resp is None or len(resp) < 2:
                        print('PUBLISH : Invalid response', resp)
                        return
...
def signUp(CL_ADDRESS):

    CL_NAME = input("\tEnter your username: ")
        #add check if name is already used
    cl.set_client_name(CL_NAME)        

    CL_PASSWORD = input("\tEnter your password: ")
    cl.set_client_password(CL_PASSWORD)
	
    print("\tYour IP address has been detected as", cl.get_client_IP_address())
...
def logIn(CL_ADDRESS):
    ...
    client_UDP = input("\tPlease enter the desired UDP port [4000, 5999]: ")
        #add check if UDP port is already used
    while not inputChecker.checkUDP(client_UDP):
        client_UDP = input("\tPlease enter the desired UDP port: ")
    cl.set_client_UDP_port(client_UDP)
	
    client_TCP = input("\tPlease enter the desired TCP port [6000, 7999]: ")
    #add check if TCP port is already used
    while not inputChecker.checkTCP(client_TCP):
        client_TCP = input("\tPlease enter the desired TCP port: ")
    cl.set_client_TCP_port(client_TCP)


    return 


userCheck = input("\nWould you like to Log In (press 1) or Sign Up (press 2)?")
...
if userCheck == "1":
    ...
    #print(cl.execServerCommand(['CHECK-IP','0', cl.ipaddress]))
    cl_UDP = cl.get_client_UDP_port()
    response = cl.execServerCommand(['CHECK-UDP','0', cl_UDP])
    while response is None or response[0] == "UDP-ERROR":
        cl_UDP = input("UDP is taken, please enter another UDP port number: ")
        response = cl.execServerCommand(['CHECK-UDP','0', cl_UDP])
    cl.set_client_UDP_port(cl_UDP)
            
    cl.execServerCommand(['AUTHENTICATE', '0', cl.get_client_name(), cl.get_client_password()])
    #print("\nWelcome back ", cl.Name)

elif userCheck == "2":
        print(f'\nChecking name:', cl.get_client_name())
		resp0 = cl.execServerCommand(['CHECK-NAME','0', cl.get_client_name()])
		while resp0[0] == "NAME-ERROR"
		    print(f"Name {cl.get_client_name()} is already in use.")
			newName = input("Please input another name: ")
			if resp0[0] == "NAME-CONFIRM"
			    cl.set_client_name(newName)
		
        print(f'\nChecking ip:', cl.execServerCommand(['CHECK-IP','0', cl.get_client_IP_address()]))
        #print(f'\nchecking ip:', cl.execServerCommand(['CHECK-NAME','0', cl.Name]))
        print(f'\nChecking UDP port number:', cl.execServerCommand(['CHECK-UDP','0', cl.get_client_UDP_port()]))
        print(f'\nChecking TCP port number:', cl.execServerCommand(['CHECK-TCP','0', cl.get_client_TCP_port()]))

        cl.execServerCommand(['REGISTER', '0', cl.get_client_name(), cl.get_client_password(), cl.get_client_IP_address(), cl.get_client_UDP_port(), cl.get_client_TCP_port()])
        #print("\nWelcome ", cl.Name)


print("\nUDP target IP:", serverIP)
print("UDP target port:", UDP_PORT)
print("\nUDP source IP:", cl.get_client_IP_address())
print("UDP source port:", cl.get_client_UDP_port())
print(" ")
